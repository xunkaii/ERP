...
<script src="${path}/gantt-master/dist/frappe-gantt.js"></script>
				...
				<div class="modal-body">
					<form class="form" method="post" id="frm">
						...
						<div class="input-group mb-3">
			            	...
			            	<input type="text" class="form-control item_name" name="item_name" data-toggle="dropdown" autocomplete="off" readonly>
	...
	$(document).ready(function(){
		...
		if(job_level == '점주' || job_level == '매니저' || job_level == '팀장' || job_level == '사장') {
			...
			$("[name=item_name]").attr("readonly",false)
			$("#uptBtn").show()
			$("#delBtn").show()
		}else{
			...
			$("[name=item_name]").attr("readonly",true)
			$("#uptBtn").hide()
			$("#delBtn").hide()
		}
	...
	$(document).on('click', '.prdResults .dropdown-item', function() {
	    ...
	    var selectedNum = $(this).siblings('.productNum').val();
